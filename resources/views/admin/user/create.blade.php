            ...
            <div class="tile-body">
                <div class="row">
                    <div class="form-group col-md-12">
                        ...
                        <select name="id_grup" class="form-control {{ $errors->has('id_grup') ? 'is-invalid' : '' }}" id="grup" {{ Auth::user()->role == role_admin_sistem() ? '' : 'disabled' }}>
                            ...
                            @else
                              @foreach($grup as $data)
                              <option value="{{ $data->id_grup }}" {{ Auth::user()->id_grup == $data->id_grup ? 'selected' : '' }}>{{ $data->nama_grup }}</option>
                              @endforeach
                            @endif
                        </select>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <select name="id_kantor" class="form-control {{ $errors->has('id_kantor') ? 'is-invalid' : '' }}" id="kantor" {{ Auth::user()->role == role_admin_sistem() ? old('id_kantor') != null ? '' : 'disabled' : '' }}>
                          @if(Auth::user()->role == role_admin_sistem())
                            @if(old('id_kantor') != null)
                              <option value="" disabled>--Pilih--</option>
                              <option value="0" {{ old('id_kantor') == 0 ? 'selected' : '' }}>ADMIN GRUP</option>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <input type="text" name="nama" class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" value="{{ old('nama') }}" placeholder="Masukkan Nama">
                        @if($errors->has('nama'))
